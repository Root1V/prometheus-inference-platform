...
| RM-78 | Idempotency keys for inference requests | in-progress | Without them a client retry is always a new, billable generation, so an SDK can only retry where the platform proves nothing ran — which excludes the commonest case, retrying after its own timeout. Raised by the Axonium SDK team |
