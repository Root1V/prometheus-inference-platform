...
| RM-73 | Billing traceability per replica + per-model metrics rollup | in-progress | `usage_events` records which replica served alongside the model id and slug; metrics gain a per-model aggregate so the dashboard can show a model's throughput rather than N loose instance rows |
