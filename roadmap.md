...
| RM-72 | Gateway: intelligent load balancing across replicas | done | Absorbs RM-58. Least-in-flight, normalised by each backend's reported capacity, verified live. Session affinity deliberately not built — see the detail section for the measurements behind that call Least-in-flight first (engine-agnostic), then free-slot/queue-aware from llama.cpp's `/slots` and `/metrics`, then optional session affinity — strategy configurable per model |
