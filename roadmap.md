...
| RM-68 | Alembic migrations for the gateway database | todo | `create_tables()` only ever creates missing tables, never alters them — RM-60 already needed a hand-rolled `ALTER TABLE`, and RM-70 changes several tables holding real data |
| RM-69 | Replica failover, active health checks, and group-based pricing | todo | Today a replica adds neither throughput nor fault tolerance: the circuit breaker is checked only on the deterministic pick, retries hammer the same dead instance, and routing by instance id escapes both pricing and the spend cap |
| RM-70 | Model/instance identity: immutable slug, opaque ids, per-model label | todo | One string is currently the catalog id, the process id, the client-facing name and the billing/scope key; splits them per `docs/model-identity-proposal.md`, with old ids kept as aliases and grants reissued |
| RM-71 | Replica UX: "Add instance" flow and a model-level scope picker | todo | Creating a replica should ask only node/engine/port and auto-increment the label; the scope picker should list models with a replica-count pill, never individual instances |
| RM-72 | Gateway: intelligent load balancing across replicas | todo | Absorbs RM-58. Least-in-flight first (engine-agnostic), then free-slot/queue-aware from llama.cpp's `/slots` and `/metrics`, then optional session affinity — strategy configurable per model |
