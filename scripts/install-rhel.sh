...
echo "       # PRM-102: auth-service publishes no port — its admin API is reachable"
echo "       # only from inside the pod, so this runs in the container."
echo "       AUTH_ADMIN_API_KEY=\$(grep ^AUTH_ADMIN_API_KEY auth-service/.env | cut -d= -f2)"
echo "       podman exec prometheus-auth curl -s --cacert /run/secrets/auth_tls.crt \\"
echo "         -X POST https://localhost:9000/admin/clients \\"
