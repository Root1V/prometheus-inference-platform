...
from prometheus_manager.lifecycle import (
    LifecycleError,
    _verify_pid_file,
    deregister_instance,
    pause_instance,
...
class TestACSuccessfulStart:
    ...
    def test_AC4_start_builds_correct_command(self, default_config, populated_registry):
        ...
        with (
            ...
            patch("prometheus_manager.lifecycle.time.sleep"),
            patch("prometheus_manager.lifecycle.time.monotonic", side_effect=[0, 1, 999]),
        ):
...
class TestACStop:
    ...
    def test_AC6_stop_sends_sigterm(self, default_config, populated_registry):
        ...
        with (
            patch("prometheus_manager.lifecycle._find_running", return_value=mock_ps),
            patch("prometheus_manager.lifecycle.psutil.Process", return_value=mock_psutil_proc),
        ):
...
class TestACPause:
    ...
    def test_AC6b_sigstop_sent(self, default_config):
        ...
        with (
            patch("prometheus_manager.lifecycle._find_running", return_value=mock_ps),
            patch("prometheus_manager.lifecycle.os.kill") as mock_kill,
        ):
...
class TestACDeregister:
    ...
    def test_AC6d_stops_and_removes(self, default_config, populated_registry):
        ...
        with patch("prometheus_manager.lifecycle.stop_instance") as mock_stop:
...
class TestDiscoveryAutoToggle:
    ...
    def test_AC8_start_sets_discovery_true(self, default_config, populated_registry):
        ...
        with (
            patch("prometheus_manager.lifecycle._find_running", return_value=None),
            patch("prometheus_manager.lifecycle._find_free_port", return_value=9090),
            patch("prometheus_manager.lifecycle.subprocess.Popen") as mock_popen,
            patch("prometheus_manager.lifecycle.httpx.get") as mock_get,
            patch("prometheus_manager.lifecycle.scan", return_value=[mock_state]),
        ):
            mock_proc = MagicMock()
    ...
    def test_AC9_stop_sets_discovery_false(self, default_config, populated_registry):
        ...
        with (
            ...
            patch("prometheus_manager.lifecycle._verify_pid_file"),
            patch("prometheus_manager.lifecycle.psutil.Process") as mock_proc_cls,
        ):
            mock_proc = MagicMock()
