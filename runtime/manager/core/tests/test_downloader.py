...
from prometheus_manager_core.downloader import (
    DownloadError,
    DownloadState,
    _sha256,
    _validate_filename,
...
def _patch_hf(
    ...
    return (
        (
            patch("prometheus_manager_core.downloader.hf_hub_url", return_value=url),
            patch(
                "prometheus_manager_core.downloader.build_hf_headers",
                return_value={"user-agent": "test/1.0"},
            ),
            patch("prometheus_manager_core.downloader._requests", mock_req),
        ),
        mock_req,
        resp,
    )
...
class TestDownloaderAC20:
    ...
    def test_AC20_hub_url_error_raises_download_error(self, tmp_path: Path):
        ...
        with (
            patch(
                "prometheus_manager_core.downloader.hf_hub_url",
