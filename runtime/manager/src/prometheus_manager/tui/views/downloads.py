...
class DownloadsView(Vertical):
    ...
    def refresh_data(self, downloads: list) -> None:
        ...
        active = [i for i, d in enumerate(downloads) if d.status in _ACTIVE_STATUSES]
        history = [i for i, d in enumerate(downloads) if d.status in _HISTORY_STATUSES]
    ...
    def _add_row(self, table: DataTable, dl: object, idx: int) -> None:
