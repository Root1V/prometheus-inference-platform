...
class InstancesView(Vertical):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._running_by_id: dict[str, ProcessState] = {}
    ...
    def _update_detail(
        self, model_id: str, state: ProcessState | None, entry: RegistryEntry | None
    ) -> None:
        ...
        # ── Identity ──────────────────────────────────────────────────────
        if is_orphan:
            assert state is not None  # implied by is_orphan
            identity_lines = [
    ...
    async def _fetch_live_data(
        self, model_id: str, port: int, pid: int, entry: RegistryEntry | None
    ) -> None:
    ...
    def refresh_data(
        self, states: list[ProcessState], registry_entries: dict[str, RegistryEntry]
    ) -> None:
    ...
    def action_start(self) -> None:
        cast("ManagerApp", self.app).action_start_selected()

    def action_stop(self) -> None:
        cast("ManagerApp", self.app).action_stop_selected()

    def action_pause(self) -> None:
        cast("ManagerApp", self.app).action_pause_selected()

    def action_resume(self) -> None:
        cast("ManagerApp", self.app).action_resume_selected()

    def action_restart(self) -> None:
        cast("ManagerApp", self.app).action_restart_selected()

    def action_deregister(self) -> None:
        cast("ManagerApp", self.app).action_deregister_selected()
